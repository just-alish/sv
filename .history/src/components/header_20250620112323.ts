...
export function createHeader() {
      ...
      <label for='themeCheckbox'>
        <div class='themeInsides'>
          <svg id="moon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 213 213">
            ...
            <path class="cls-1" d="M150,46a105.2,105.2,0,0,0-12.62.76,71.5,71.5,0,1,1-90.62,90.62A105.2,105.2,0,0,0,46,150,104,104,0,1,0,150,46Z" transform="translate(-43.5 -43.5)"/>
            <path class="cls-1" d="M150,46a105.2,105.2,0,0,0-12.62.76,71.5,71.5,0,1,1-90.62,90.62A105.2,105.2,0,0,0,46,150,104,104,0,1,0,150,46Z" transform="translate(-43.5 -43.5)"/>
          </svg>
        </div>
        <div class='themeInsides'>
          <svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 212 212">
            ...
            <circle class="cls-1" cx="105.5" cy="106" r="53.5"/>
            <rect class="cls-2" x="103.5" width="5" height="40"/><rect class="cls-2" x="103.5" y="172" width="5" height="40"/>
            <rect class="cls-2" x="208.81" y="69.19" width="5" height="40" transform="translate(80.46 -167.3) rotate(45)"/>
            <rect class="cls-2" x="87.19" y="190.81" width="5" height="40" transform="translate(130.84 -45.67) rotate(45)"/>
            <rect class="cls-2" x="87.19" y="69.19" width="5" height="40" transform="translate(-81.3 45.54) rotate(-45)"/>
            <rect class="cls-2" x="208.81" y="190.81" width="5" height="40" transform="translate(-131.67 167.16) rotate(-45)"/>
            <rect class="cls-2" x="62" y="130" width="5" height="40" transform="translate(-130 170.5) rotate(-90)"/>
            <rect class="cls-2" x="234" y="130" width="5" height="40" transform="translate(42 342.5) rotate(-90)"/>
          </svg>
        </div>
      </label>
  ...
  let themeBtnAngle = 0;
  const labels = headerThemeBtn.querySelectorAll('.themeInsides');

  if (labels) {
    labels.forEach((label) => {
      label.addEventListener('click', () => {
        themeBtnAngle -= 180;
        rotate(headerThemeBtn, themeBtnAngle, { duration: 400, easing: 'ease-out' })
      });
    });
  }

  const checkbox = headerThemeBtn.querySelector<HTMLInputElement>('.themeInsides');

  if (checkbox) {
    checkbox.addEventListener('change', () => {
      document.querySelector('.light-bg')?.classList.remove('no-transition');
      if (checkbox.checked) {
        console.log('light');
        document.querySelector('.light-bg')?.classList.add('visible');
      } else {
        console.log('dark');
        document.querySelector('.light-bg')?.classList.remove('visible');
      }
    });
  }
