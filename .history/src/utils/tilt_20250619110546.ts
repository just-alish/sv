export function addSpringTilt(element: HTMLElement, {
  ...
  const updateTransform = (e: MouseEvent) => {
    ...
    const rotateY = ((x / rect.width ) - 0.5) * (maxRotateW ?? (64 * 20 / rect.width));
    const rotateX = ((y / rect.height) - 0.5) * -(maxRotateH ?? (64 * 20 / rect.height));
  ...
  element.addEventListener('mouseenter', () => {
    rect = element.getBoundingClientRect(); // Only compute *once*
  });
